/**
 * @doscientos/astro-reveal
 * Beautiful scroll-reveal animations for Astro
 *
 * @see https://github.com/doscientos/astro-reveal
 */
...
export function init() {
  const elements = document.querySelectorAll<HTMLElement>('[data-reveal]:not([data-init])');
  ...
  for (const el of elements) {
    ...
    if (distance) {
      ...
      el.style.setProperty('--reveal-distance', distanceMap[distance] || `${distance}px`);
    }
